...
export default async function MonthPage({
  ...
  const { channel: channelName, month } = await params;
  const channel = await getChannelByName(decodeURIComponent(channelName));
  if (!channel) notFound();
  ...
  return (
    <section className="mx-auto max-w-4xl px-6 py-16">
      <p className="text-sm text-muted-foreground">
        ...
        /{" "}
        <Link href={`/members/archive/${channel.name}`} className="underline underline-offset-4">
          #{channel.name}
        </Link>{" "}
        / {monthLabel(month)}
      </p>
